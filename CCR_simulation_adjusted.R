...
# This script reproduces the adjusted simulation study for the CCR method.
...
make_dummy_matrix <- function(latent) {
  ...
  colnames(X) <- paste0("Zx", 1:10)
  
  X_factor <- data.frame(
    lapply(X, function(x) factor(x, levels = sort(unique(x))))
  )
  colnames(X_factor) <- paste0("Zx", 1:10)
...
# Generate adjustment covariates used in the adjusted simulation setting.
...
get_lm_start <- function(Z, Y) {
  coef0 <- tryCatch(
    coef(lm.fit(x = cbind(Intercept = 1, Z), y = Y)),
    error = function(e) c(mean(Y), 0, 0)
  )
  coef0[is.na(coef0)] <- 0
  coef0
}

get_glm_start <- function(Z, Y) {
  init_prob <- min(max(mean(Y), 1e-4), 1 - 1e-4)
  coef0 <- tryCatch(
    glm.fit(x = cbind(Intercept = 1, Z), y = Y, family = binomial())$coefficients,
    error = function(e) c(qlogis(init_prob), 0, 0)
  )
  coef0[is.na(coef0)] <- 0
  coef0
}

# Fit adjusted CCR for a Gaussian outcome.
# The parameter vector is (beta0, beta, w1, ..., wp, alpha1, alpha2).
fit_ccr_gaussian <- function(X, Z, Y, penalty = 10) {
  objective <- function(theta) {
    b0 <- theta[1]
    b <- theta[2]
    w <- theta[3:(p + 2)]
    alpha <- theta[(p + 3):(p + 4)]
    eta <- as.numeric(b0 + b * (X %*% w) + Z %*% alpha)
    mean((Y - eta)^2) + penalty * (sum(w) - 1)^2
  }
  
  coef0 <- get_lm_start(Z, Y)
  init <- c(coef0[1], sum(beta_true), rep(1 / p, p), coef0[2:3])
...
# Fit adjusted CCR for a binary outcome.
# The logistic likelihood includes both the CCR index and adjustment covariates.
fit_ccr_binary <- function(X, Z, Y, penalty = 1) {
  objective <- function(theta) {
    b0 <- theta[1]
    b <- theta[2]
    w <- theta[3:(p + 2)]
...
# Run one adjusted simulation setting defined by outcome type and exposure correlation rho.
run_one_setting <- function(outcome = c("gaussian", "binary"), rho) {
  ...
  alpha_ccr_mat <- matrix(NA_real_, n_sim, 2)
  ...
  for (sim in 1:n_sim) {
    ...
    Z <- generate_covariates(n)
    
    # Generate the outcome under the adjusted CCR data-generating model.
    if (outcome == "gaussian") {
      Y <- as.numeric(beta0_gaussian + X_dummy %*% beta_true + Z %*% alpha_true + rnorm(n))
      fit <- withCallingHandlers(
        fit_ccr_gaussian(X_dummy, Z, Y),
        warning = function(w) {
      ...
      eta <- as.numeric(beta0_binary + X_dummy %*% beta_true + Z %*% alpha_true)
      prob <- 1 / (1 + exp(-eta))
      Y <- rbinom(n, 1, prob)
      fit <- withCallingHandlers(
        fit_ccr_binary(X_dummy, Z, Y),
        warning = function(w) {
          log_warning(sim, NA, w)
          invokeRestart("muffleWarning")
        }
    ...
    beta_hat <- fit$par[2]
    w_hat <- fit$par[3:(p + 2)]
    alpha_hat <- fit$par[(p + 3):(p + 4)]
    
    beta_ccr_mat[sim, ] <- beta_hat * w_hat
    alpha_ccr_mat[sim, ] <- alpha_hat
    ...
    for (k in 1:K) {
      ...
      if (outcome == "gaussian") {
        fit_cv <- withCallingHandlers(
          fit_ccr_gaussian(X_train, Z_train, Y_train),
          warning = function(w) {
            log_warning(sim, k, w)
            invokeRestart("muffleWarning")
          }
        ...
        pred_ccr[test] <- as.numeric(
          fit_cv$par[1] +
            fit_cv$par[2] * (X_test %*% fit_cv$par[3:(p + 2)]) +
            Z_test %*% fit_cv$par[(p + 3):(p + 4)]
        )
      } else {
        if (length(unique(Y_train)) < 2) next
        fit_cv <- withCallingHandlers(
          fit_ccr_binary(X_train, Z_train, Y_train),
          warning = function(w) {
            log_warning(sim, k, w)
            invokeRestart("muffleWarning")
          }
  ...
  alpha_res <- calc_bias_rmse(alpha_ccr_mat, alpha_true)
...
make_tables <- function(res) {
  ...
  tables <- list(
    ...
    alpha_bias = data.frame(
      Covariate = c("Z1", "Z2"),
      CCR = res$alpha$bias
    ),
    alpha_rmse = data.frame(
      Covariate = c("Z1", "Z2"),
      CCR = res$alpha$rmse
    ),
    zero = data.frame(
...
# Run all adjusted simulation settings and export the results.
all_results <- list()
wb <- createWorkbook()
...
saveWorkbook(wb, "simulation_adjusted_results.xlsx", overwrite = TRUE)
